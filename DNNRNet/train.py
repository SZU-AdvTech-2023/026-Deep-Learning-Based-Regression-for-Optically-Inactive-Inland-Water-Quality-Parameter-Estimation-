...
def train_model(model, train_loader, val_loader, epochs_start, loss_start, num_epochs, learning_rate):
    ...
    # 定义学习率调度器
    # scheduler = StepLR(optimizer, step_size=100, gamma=0.1)  # 在每100个epoch后将学习率乘以0.1
    ...
    writer = SummaryWriter('logs_DNNR_Adam_SmoothMSE')
...
def train_model_LBFGS(model, train_loader, val_loader, epochs_start, loss_start, num_epochs, learning_rate):
    ...
    # 定义学习率调度器
    # scheduler = StepLR(optimizer, step_size=20, gamma=0.1)  # 在每100个epoch后将学习率乘以0.1
    ...
    # 创建一个 SummaryWriter 实例，指定记录的目录
    writer = SummaryWriter('logs-LBFGS-SmoothMSE-ACMix')
    def closure():
        optimizer.zero_grad()
