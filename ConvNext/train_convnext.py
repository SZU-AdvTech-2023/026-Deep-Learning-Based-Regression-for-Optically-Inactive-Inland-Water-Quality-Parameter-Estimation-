...
parser = argparse.ArgumentParser(description='Convnext_args')
parser.add_argument('--train_batch_size', type=int, default=60, help='train_batch_size')
...
parser.add_argument('--lbfgs', type=bool, default=True, help='use lbfgs')
...
def train_model(model, train_loader, val_loader, epochs_start, loss_start, num_epochs, learning_rate):
    ...
    # criterion = DNNRNet.R2ReciprocalLoss()
    ...
    writer = SummaryWriter('logs-ConvNext')
    for epoch in range(num_epochs - epochs_start):  # 从预训练模型的epoch开始训练
        ...
        if epoch_loss < best_loss:
            ...
            torch.save({'model': model.state_dict(), 'epoch': epoch + epochs_start + 1, 'loss': epoch_loss},
                       f"{args.save_model_path}/{element_name}/ConvNext_best.pth")

        if (epoch + epochs_start + 1) % 10 == 0:
            torch.save({'model': model.state_dict(), 'epoch': epoch + epochs_start + 1, 'loss': epoch_loss},
                       f"{args.save_model_path}/{element_name}/ConvNext_last.pth")
            loss_train, rmse_train, mae_train, r2_train = validate_model(model, train_loader, criterion)
            loss_val, rmse_val, mae_val, r2_val = validate_model(model, val_loader, criterion)
            print(f'Train_Data:\nLoss:{loss_train} RMSE: {rmse_train} MAE:{mae_train} R2:{r2_train}')
            print(f'Val_Data:\nLoss:{loss_val} RMSE: {rmse_val} MAE:{mae_val} R2:{r2_val}')
...
def train_model_LBFGS(model, train_loader, val_loader, epochs_start, loss_start, num_epochs, learning_rate):
    # criterion = nn.MSELoss()  # 使用均方误差损失函数
    criterion = nn.HuberLoss(delta=1)  # HuberLoss delta=1时为平滑MSE
    optimizer = optim.LBFGS(model.parameters(), lr=learning_rate, history_size=50)
    # 定义学习率调度器
    scheduler = StepLR(optimizer, step_size=30, gamma=0.3)  # 在每100个epoch后将学习率乘以0.1
    ...
    writer = SummaryWriter('logs-ConvNext-LBFGS')
    for epoch in range(num_epochs - epochs_start):  # 从预训练模型的epoch开始训练
        model.train()
        running_loss = 0.0
...
def to_torchtensor(element_id, img_path):  # 将文件夹中的图片和对应坐标点的excel中的水体参数值，转换为tensor格式
    image_list_train = []  # 训练集图片矩阵的列表
    image_list_val = []  # 验证集图片矩阵的列表
    y_train1 = []  # 临时存放训练集y值的列表
    y_val1 = []  # 临时存放验证集y值的列表
    bands = [132, 133, 134, 135, 136, 137, 138, 139, 140, 141, 142, 143, 144, 145, 146, 147, 148, 149, 150, 151, 152,
             153, 154, 155, 156, 157, 158, 159, 160, 161, 162, 163, 164, 165, 166, 167, 168, 169, 170, 171, 172, 174,
             175, 176, 177, 178, 179, 180, 184, 185, 186, 187, 188, 198, 199, 200, 201]
    if element_id == 10:
    ...
    for filename in os.listdir(img_path_train):
        img_path2 = os.path.join(img_path_train, filename)
        img = imageio.imread(img_path2)  # 使用 imageio 库读取 TIFF 图像数据
        # img = img[bands, :, :]  # 使用切片提取对应的波段
        img_tensor = torch.from_numpy(img)  # 将图像数据转换为 PyTorch 张量
        image_list_train.append(img_tensor)  # 加入到列表中
        x_train = torch.stack(image_list_train)  # 将图像数据转换为 PyTorch 张量
        str_img_name = os.path.splitext(filename)[0]  # 图片名去掉文件后缀部分

        str_img_name = str_img_name.split('_')  # 通过_划分文件名为多个部分
        str_img_name = str_img_name[0]

        # 去除括号和逗号，并分割字符串
        # 从文件名中获取采样点坐标
        str_img_name = str_img_name.replace('(', '').replace(')', '')
        x, y = str_img_name.split(',')
        x = int(x)
        y = int(y)
        # 构建元组 (x, y)
        data_tuple = (x, y)
        y_train1.append(search_xy(args.excel_path, data_tuple, args.element_id))  # 将根据坐标获取的水质参数数据加入y_train

    y_train = torch.from_numpy(np.array(y_train1)).reshape(-1, 1)  # 转换为将训练集y值转换为tensor格式

    for filename in os.listdir(img_path_val):
        img_path3 = os.path.join(img_path_val, filename)
        img = imageio.imread(img_path3)  # 使用 imageio 库读取 TIFF 图像数据
        # img = img[bands, :, :]  # 使用切片提取对应的波段
        img_tensor = torch.from_numpy(img)  # 将图像数据转换为 PyTorch 张量
        image_list_val.append(img_tensor)  # 加入到列表中
        x_val = torch.stack(image_list_val)  # 将图像数据转换为 PyTorch 张量
        str_img_name = os.path.splitext(filename)[0]  # 图片名去掉文件后缀部分
        # 去除括号和逗号，并分割字符串
        # 从文件名中获取采样点坐标
        str_img_name = str_img_name.replace('(', '').replace(')', '')
        ...
        # 构建元组 (x, y)
        data_tuple = (x, y)
        y_val1.append(search_xy(args.excel_path, data_tuple, args.element_id))  # 将根据坐标获取的水质参数数据加入y_train

    y_val = torch.from_numpy(np.array(y_val1)).reshape(-1, 1)  # 转换为将训练集y值转换为tensor格式
...
if __name__ == '__main__':
    ...
    loss_start = 0.1
    if args.pretrained:
        state_dict = torch.load(f'{args.pretrained_path}{element_name}/ConvNext_Best.pth')
    ...
    # 调用训练函数开始训练
    if args.lbfgs:
        train_model_LBFGS(model, train_loader, val_loader, epochs_start, loss_start, args.num_epochs,
                          args.learning_rate)
    else:
